...
const totalPagar = computed(() => {
    return props.carrito.reduce((total, guitarra) => total + (guitarra.cantidad * guitarra.precio), 0);
});
</script>
                ...
                <nav class="col-md-6 a mt-5 d-flex align-items-start justify-content-end">
                    <div class="carrito">
                        ...
                        <div id="carrito" class="bg-white p-3">
                            <p v-if="carrito.length == 0" class="text-center mb-0">El carrito esta vacio</p>
                            <div v-else>
                                        ...
                                        >
                                            ...
                                            <td class="flex align-items-start gap-4">
                                                <button type="button" class="btn btn-dark"
                                                    @click="$emit('decrementar-cantidad', guitarra.id)">
                                                    -
                                                </button>
                                                {{ guitarra.cantidad }}
                                                <button type="button" class="btn btn-dark"
                                                    @click="$emit('incrementar-cantidad', guitarra.id)">
                                                    +
                                                </button>
                                            </td>
                                            <td>
                                                <button 
                                                    ...
                                                    type="button"
                                                    @click="$emit('eliminar-producto', guitarra.id)"
                                                >
                                                    X
                                                </button>
                                            </td>
